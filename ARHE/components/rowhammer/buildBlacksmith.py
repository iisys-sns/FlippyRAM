...
cppSnippet = os.path.join(script_dir, '../../data/tmp/generated_snippet_blacksmith.cpp')
hppSnippet = os.path.join(script_dir, '../../data/tmp/generated_snippet_blacksmith.hpp')
...
errPath = os.path.join(script_dir, '../../data/errors/buildBlacksmith.txt')
...
if args.threshold:
    globalDefines = f"#define THRESH {threshold}\n\n#define NUM_BANKS {num_banks}\n\n#define NUM_RANKS {num_ranks}\n\n#define DIMM {num_dimms}\n\n#define CHANNEL {num_channels}"
    with open(hppSnippet, "w") as file:
        file.write(globalDefines)
else:
    dram_fns = []
    amdre_fns = []
    trrespass_fns = []
    ...
    # Note that def_fns (Default Functions are defined if the injection fails for any reason, those functions could still be used for blacksmith)
    if (fNum == 5):
        row_fn = 0x3ffc0000
