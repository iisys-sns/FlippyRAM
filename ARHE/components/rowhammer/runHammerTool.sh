...
root_path=$ROOT_PATH
bitFlipLog="$root_path/data/bit-flip-reports-hammertool.txt"
bitFlipCounter="$root_path/data/tmp/hammertoolBitFlipsNum.tmp"
...
while [ "$runtime" -gt 60 ]; do
	...
	runOutputFile="$root_path/data/logs/hammertool-$(date +%Y-%m-%d_%H-%M-%S).log"
	# Write information that the experiment is starting
	echo "HammerTool Started | [$(date +%Y-%m-%d_%H-%M-%S)] | timestamp: $(date +%s)" >> "$runOutputFile"
	echo "----------------------------------------------------" >> "$runOutputFile"
	echo "Starting HammerTool. Logging output to $runOutputFile" >> "$bitFlipLog"
  ...
  #   within the loop, therefore the "strange" variable handling using files
	...
	echo "timeout --kill-after=5 $runtime stdbuf -oL ./bin/hammertool --physical --sets 100 --multithreading --quiet --importConfig config.cfg --hammerDoubleSide 2>&1" >> "$runOutputFile"
	timeout --kill-after=5 $runtime stdbuf -oL ./bin/hammertool --physical --sets 100 --multithreading --quiet --importConfig config.cfg --hammerDoubleSide 2>&1 | while IFS= read -r line;
	do
		...
		if [[ "$line" == *"Bit Flip found at offset"* ]];then
		...
		if [[ "$line" == *"monitored command dumped core"* ]];then
			echo -e "${BLACK_TXT}${PURPLE_BG} Rowhammer ${COLOR_RESET} Hammertool Dumped Core. Stopping experiment..."
			echo "true" > "$root_path/data/tmp/rowhammerTool_Dumped_core.tmp"
			killall -9 "hammertool" > /dev/null 2>&1
		fi
		...
		if [ "$fileSize" -ge "$FILE_SIZE_LIMIT" ]; then
			echo -e "${BLACK_TXT}${PURPLE_BG} Rowhammer ${COLOR_RESET} Hammertool exceeded maximum log file size. Stopping experiment..."
			killall -9 "hammertool" > /dev/null 2>&1
			exit 1
		fi
	...
	echo "----------------------------------------------------" >> "$runOutputFile"
	echo "HammerTool Ended | [$(date +%Y-%m-%d_%H-%M-%S)] | timestamp: $(date +%s)" >> "$runOutputFile"
	dumped_core=$(cat "$root_path/data/tmp/rowhammerTool_Dumped_core.tmp")
	if [ "$dumped_core" = "true" ];then
		echo  "[$(date +%Y-%m-%d_%H-%M-%S)] | Hammertool Dumped Core!" >> "$runOutputFile"
		break
	fi
	runtime=$(($end_timestamp - $(date +%s)))
done
