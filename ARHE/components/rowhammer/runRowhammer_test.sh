...
root_path=$ROOT_PATH
bitFlipLog="$root_path/data/bit-flip-reports-rowhammertest.txt"
bitFlipCounter="$root_path/data/tmp/rowhammertestBitFlipsNum.tmp"
...
cd $root_path/Tools/rowhammer-test
...
while [ "$runtime" -gt 60 ]; do
	...
	runOutputFile="$root_path/data/logs/rowhammertest-$(date +%Y-%m-%d_%H-%M-%S).log"
	# Write information that the experiment is starting
	echo "Rowhammer-test Started | [$(date +%Y-%m-%d_%H-%M-%S)] | timestamp: $(date +%s)" >> "$runOutputFile"
	echo "----------------------------------------------------" >> "$runOutputFile"
	echo "Starting Rowhammer-test. Logging output to $runOutputFile" >> "$bitFlipLog"
  ...
  #   within the loop, therefore the "strange" variable handling using files
	...
	echo "timeout --kill-after=5 $runtime stdbuf -oL ./rowhammer_test 2>&1" >> "$runOutputFile"
	timeout --kill-after=5 $runtime stdbuf -oL ./rowhammer_test 2>&1 | while IFS= read -r line;
	do
		...
		if [[ "$line" == *"monitored command dumped core"* ]];then
			echo -e "${BLACK_TXT}${PURPLE_BG} Rowhammer ${COLOR_RESET} Rowhammer-test Dumped Core. Stopping experiment..."
		...
		if [ "$fileSize" -ge "$FILE_SIZE_LIMIT" ]; then
			echo -e "${BLACK_TXT}${PURPLE_BG} Rowhammer ${COLOR_RESET} Rowhammer-test exceeded maximum log file size. Stopping experiment..."
			killall -9 "rowhammer_test" > /dev/null 2>&1
			exit 1
		fi
	...
	echo "----------------------------------------------------" >> "$runOutputFile"
	echo "Rowhammer-test Ended | [$(date +%Y-%m-%d_%H-%M-%S)] | timestamp: $(date +%s)" >> "$runOutputFile"
	dumped_core=$(cat "$root_path/data/tmp/rowhammerTool_Dumped_core.tmp")
    if [ "$dumped_core" = "true" ];then
		echo  "[$(date +%Y-%m-%d_%H-%M-%S)] | Rowhammer.js Dumped Core!" >> "$runOutputFile"
		break
	fi
	runtime=$(($end_timestamp - $(date +%s)))
done
