...
void Logger::log_info(const std::string &message, bool newline) {
  ...
  std::cout << FC_CYAN "[+] " << message << F_RESET;
  if (newline) {
    instance.logfile << "\n";
    std::cout << std::endl;
  }
...
void Logger::log_highlight(const std::string &message, bool newline) {
  ...
  instance.logfile << F_RESET;
  std::cout << FC_MAGENTA << FF_BOLD << "[+] " << message << F_RESET;
  if (newline) {
    instance.logfile << "\n";
    std::cout << std::endl;
  }
...
void Logger::log_error(const std::string &message, bool newline) {
  ...
  instance.logfile << F_RESET;
  std::cout << FC_RED "[-] " << message << F_RESET;
  if (newline) {
    instance.logfile << "\n";
    std::cout << std::endl;
  }
...
void Logger::log_bitflip(volatile char *flipped_address, uint64_t row_no, unsigned char actual_value,
  ...
  instance.logfile << F_RESET;
  std::cout << FC_GREEN << "Bit Flip Found! Check The Details inside stdout txt file!" << F_RESET;
  if (newline) {
    instance.logfile << "\n";
    std::cout << std::endl;
  }
...
void Logger::log_success(const std::string &message, bool newline) {
  ...
  instance.logfile << F_RESET;
  std::cout << FC_GREEN << message << F_RESET;
  if (newline) {
    instance.logfile << "\n";
    std::cout << std::endl;
  }
...
void Logger::log_failure(const std::string &message, bool newline) {
  ...
  instance.logfile << F_RESET;
  std::cout << FC_RED_BRIGHT << message << F_RESET;
  if (newline) {
    instance.logfile << "\n";
    std::cout << std::endl;
  }
