...
#define FLIPTABLE
// ------------------------------------------------ SNS Implementation Start ------------------------------------------------
#define MAX_SAFE_SIZE 16384
// ------------------------------------------------ SNS Implementation End ------------------------------------------------
...
uint32_t crc32(uint32_t crc, const void *buf, size_t bufSize) {
    static uint32_t table[256];
    static int have_table = 0;
    uint32_t rem;
    uint8_t *p, *q;

    // Check if the buffer is null or the size is too large
    if (buf == NULL || bufSize == 0 || bufSize > MAX_SAFE_SIZE) {
        fprintf(stderr, "Error: Invalid buffer or bufSize too large\n");
        return crc;
    }

    // Ensure that adding `size` to `p` does not cause an overflow or exceed the addressable memory
    p = (uint8_t *)buf;
    if ((uintptr_t)p + bufSize < (uintptr_t)p || (uintptr_t)p + bufSize > UINTPTR_MAX) {
        fprintf(stderr, "Error: Buffer bufSize causes pointer overflow or exceeds memory limit\n");
        return crc;
    }

    q = p + bufSize;

    // Initialize the CRC table only once
    if (!have_table) {
...
char *cl_rand_gen(DRAMAddr *d_addr)
{
    ...
    for (int i = 0; i < 8; i++) {
		// ------------------------------------------------ SNS Implementation Start ------------------------------------------------
		uint64_t sum = d_addr->row + d_addr->bank + (d_addr->col + i * 8);
		cl_buff[i] = crc32(CL_SEED, &sum, sizeof(sum));  // Use custom crc32 with the same sum logic
		// ------------------------------------------------ SNS Implementation End ------------------------------------------------
    }
    return (char *)cl_buff;
}
...
void fill_stripe(DRAMAddr d_addr, uint8_t val, ADDRMapper * mapper)
{
	for (size_t col = 0; col < ROW_SIZE; col += (1 << 6)) {
		// --------------------------------- DEBUG --------------------------------
		if (col >= ROW_SIZE) {
			fprintf(stderr, "Column out of bounds in fill_row: %zu\n", col);
			return;
    	}
		// --------------------------------- DEBUG END --------------------------------
		d_addr.col = col;
...
{
	// --------------------------------- DEBUG --------------------------------
	// printf("Starting fill_row: d_addr at %p, data_patt: %d, reverse: %d\n", d_addr, data_patt, reverse);
	if (d_addr == NULL) {
		fprintf(stderr, "Error: d_addr is NULL in fill_row\n");
		return;
	}
	// printf("Contents of d_addr before modification:\n");
	// printf("d_addr: bank = %lu, row = %lu, col = %lu\n",d_addr->bank, d_addr->row, d_addr->col);
	// --------------------------------- DEBUG END --------------------------------
	...
	// --------------------------------- DEBUG --------------------------------
	// printf("Contents of d_addr after modification:\n");
	// printf("d_addr: bank = %lu, row = %lu, col = %lu\n",d_addr->bank, d_addr->row, d_addr->col);
	// --------------------------------- DEBUG END --------------------------------
}
...
{
	// ----------------------------------- DEBUG --------------------------------------------
	// printf("Scanning rows for h_patt.len: %zu, adj_rows: %zu\n", h_patt->len, adj_rows);
	if (h_patt->d_lst == NULL || h_patt->len == 0) {
		fprintf(stderr, "Error: h_patt.d_lst is NULL or empty in scan_rows\n");
		return;
	}
	// printf("Contents of h_patt.d_lst before modification:\n");
	// for (size_t i = 0; i < h_patt->len; i++) {
	// 	printf("h_patt.d_lst[%zu]: bank = %lu, row = %lu, col = %lu\n",
	// 		i, h_patt->d_lst[i].bank, h_patt->d_lst[i].row, h_patt->d_lst[i].col);
	// }
	// ----------------------------------- DEBUG END --------------------------------------------
	...
	// ----------------------------------- DEBUG --------------------------------------------
	// printf("Contents of h_patt.d_lst after modification:\n");
	// for (size_t i = 0; i < h_patt->len; i++) {
	// 	printf("h_patt.d_lst[%zu]: bank = %lu, row = %lu, col = %lu\n",
	// 		i, h_patt->d_lst[i].bank, h_patt->d_lst[i].row, h_patt->d_lst[i].col);
	// }
...
int n_sided_test(HammerSuite * suite)
{
	...
	h_patt.d_lst = (DRAMAddr *) malloc(sizeof(DRAMAddr) * h_patt.len);
	// -----------------------------------DEBUG --------------------------------------------
	if (h_patt.d_lst == NULL) {
		fprintf(stderr, "Memory allocation failed for h_patt.d_lst\n");
		exit(1);
	}
	// -----------------------------------DEBUG End --------------------------------------------
	memset(h_patt.d_lst, 0x00, sizeof(DRAMAddr) * h_patt.len);
	...
	// ------------------------------------------------ SNS Implementation Start ------------------------------------------------
	if (h_patt.d_lst != NULL) {
    	printf("Freeing h_patt.d_lst\n");
	} else {
		fprintf(stderr, "Attempting to free a NULL pointer (h_patt.d_lst)\n");
	}
	free(h_patt.d_lst);
	h_patt.d_lst = NULL;
	// ------------------------------------------------ SNS Implementation End ------------------------------------------------
