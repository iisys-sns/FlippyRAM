...
int do_samsung_utrr(uintptr_t target, int no_aggr_acts, int no_reads, int victim_count)
{
    ...
    for (int i = 14000; i < 14000 + victim_count; i++)
    {
        ...
        std::cout << "[RowPress] Row " << i+1 << " with bit flip count " << bitflips << std::endl;
    }
    ...
    if(is_verify){
        ...
        for(int i = 4; i > 0; i--){
            ...
            for(int j = 0; j < experiment_counts[4-i]; j++){
                std::cout << "[RowPress] Experiment with number of activations: " << i << " and number of reads: " << no_reads_arr[j] << std::endl;
